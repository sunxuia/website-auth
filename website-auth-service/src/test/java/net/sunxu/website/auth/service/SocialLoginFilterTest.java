...
public class SocialLoginFilterTest extends AbstractTest {
    ...
    public void requestLogin_redirectToLoginPage() throws Exception {
        var result = mockMvc.perform(get("/login/github"))
                .andExpect(status().is3xxRedirection())
                .andReturn();

        String location = result.getResponse().getHeader("Location");
        String code = (String) result.getRequest().getSession().getAttribute("oauth_state");
    ...
    public void testHandleError() throws Exception {
        mockMvc.perform(get("/login/github?error=test error"))
                .andExpect(status().is3xxRedirection())
                .andExpect(header().string("Location", "/?error=test%20error"));
    }
